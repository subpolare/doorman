...
internal class MessageProcessor
{
    ...
    public async Task HandleUpdate(Update update, CancellationToken stoppingToken)
    {
        ...
        if (chat.Type == ChatType.Private)
        {
            await _bot.SendMessage(
                chat,
                "Увы, я не отвечаю в личных сообщениях",
                ...
                if (_config.ChannelAutoBan)
                {
                    try
                    {
                        ...
                        if (_config.NonFreeChat(chat.Id))
                            await _bot.SendMessage(
                                _config.AdminChatId,
                                $"Сообщение удалено, забанил канал {message.SenderChat.Title} в чате {chat.Title} ",
                                replyParameters: fwd!,
                                cancellationToken: stoppingToken
                            );
                    }
                    catch (Exception e)
                    {
                        ...
                        if (_config.NonFreeChat(chat.Id))
                            await _bot.SendMessage(
                                admChat,
                                $"Не могу удалить или забанить сообщение от имени канала {message.SenderChat.Title} в чате {chat.Title}. Что-то пошло не по плану...",
                                cancellationToken: stoppingToken
                            );
                    }
                    return;
        ...
        if (_userManager.Approved(user.Id))
        {
            ...
            await _bot.SendMessage(
                _config.AdminChatId,
                $"AI решил, что это спам (score {score_}), но пользователь в доверенных. Возможно стоит добавить в /ham, чат {chat.Title} {Utils.LinkToMessage(chat, message.MessageId)}",
                replyParameters: fwd,
                cancellationToken: stoppingToken
            );
            return;
        ...
        if (lookalike.Count > 2)
        {
            ...
            var reason = $"Нашел слова, которые маскируются под русские: {string.Join(", ", lookalike.Take(5))}{tailMessage}";
        ...
        {
            const string reason = "В этом сообщении есть необычные буквы";
        ...
        if (SimpleFilters.TooManyEmojis(text))
        {
            ...
            const string reason = "В этом сообщении многовато эмодзи";
        ...
        if (score > -0.5 && _config.LowConfidenceHamForward && _config.NonFreeChat(message.Chat.Id))
        {
            ...
            await _bot.SendMessage(
                _config.AdminChatId,
                $"Думаю, что это НЕ спам, но могу ошибаться (score: {score}). Хорошая идея — добавить сообщение в датасет.{Environment.NewLine}Юзер {Utils.FullName(user)} из чата {chat.Title}{Environment.NewLine}{postLink}",
        ...
        switch (newChatMember.Status)
        {
            ...
            case ChatMemberStatus.Kicked or ChatMemberStatus.Restricted:
                ...
                var tailMessage = string.IsNullOrWhiteSpace(lastMessageText)
                    ? "Если его забанили за спам, а я не распознал это — киньте его сообщение сюда."
                    : $"Последним сообщением было:{Environment.NewLine}{lastMessageText}";
                var mentionAt = user.Username != null ? $"@{user.Username}" : "";
                await _bot.SendMessage(
                    _config.GetAdminChat(chatMember.Chat.Id),
                    $"В чате {chatMember.Chat.Title} юзеру {Utils.FullName(user)} {mentionAt} дали read-only или забанили. {tailMessage}"
                );
                break;
        }
    }
    ...
    private async Task DeleteAndReportMessage(Message message, string reason, CancellationToken stoppingToken)
    {
        ...
        catch (Exception e)
        {
            ...
            deletionMessagePart += ", сообщение НЕ удалено, у меня почему-то не получилось это сделать.";
        }
